...
public class Game{
	...
	public void play(int lagTime) throws InterruptedException{
		...
		while(!isGameOver){//player turn manager
			for(int a = 0; a < players.length && !isGameOver; a ++){
				...
				if(currentPlayer.getClass() == ComputerPlayer.class){
					gameBoard.computerGrid(twoHexGrid(currentPlayer.getOrientation(), currentPlayer.getPieceX(), currentPlayer.getPieceY()));
					Thread.sleep(sleepTimer);
				}
				currentPlayer.updateScore(getTurnScore(currentPlayer.getOrientation(), currentPlayer.getPieceX(), currentPlayer.getPieceY()));
				updateGrid(twoHexGrid(currentPlayer.getOrientation(), currentPlayer.getPieceX(), currentPlayer.getPieceY()));
	...
	public int[] scoreOrder(){//needs to check second lowest score if both players have a score of zero somewhere
...
public Player[] sortPlayers() {
	...
	public boolean checkLegalMove() {
		if(currentPlayer.getCurrentPiece() != null){ //if there is a piece on the mouse
