...
public class GameBoard extends JPanel implements Runnable, MouseListener, MouseMotionListener
{
	...
	private int[][] hexColor = new int[30][15]; // Contains value representing
												// color of a hex on the grid -
												// for actual game, not
												// initializing board
	...
	public GameBoard(Game game) { // JFrame made in tester class
		...
		makeHand();
		// for(int y = 0; y < 15; y++){ // For debugging prints backwards
		// because I done goofed.
		// System.out.println("");
		// for(int x = 0; x < 30;x++){
		...
		for (int y = 0; y < 15; y++) { // For debugging prints backwards because
										// I done goofed.
			for (int x = 0; x < 30; x++) {
				computerGrid[x][y] = 0;
			}
	...
	public void paintComponent(Graphics g) {
		...
		try {
			...
			g.drawLine(500, 0, 500, 800);// separates 3rd of board from rest.
											// should paint the side components
											// to the left of it.
			g.drawRect(50, 650, 400, 140);
			paintScore(g);
			g.setColor(pickColor(game.getCurrentPlayer().getCurrentPiece().getPrimaryHexagon().getColor()));
			g.drawPolygon(makeHex(width / 3 + 40, 50, 30)); // getColor or
															// whatever for
															// hexagon
			g.fillPolygon(makeHex(width / 3 + 40, 50, 30)); // getColor or
															// whatever for
															// hexagon
			if (game.getCurrentPlayer().getCurrentPiece().getPrimaryHexagon().getColor() == game.getCurrentPlayer()
					.getCurrentPiece().getSecondaryHexagon().getColor()) {
				score1 = score1 + score2;
				g.setColor(Color.BLACK);
				g2d.drawString(Integer.toString(score1), width / 3 + 80, 54); // getScore
																				// for
																				// the
																				// string
																				// or
	...
	private void paintScore(Graphics g) {
		...
		if (game.getCurrentPlayer().isHuman()) {
			for (int counter = 0; counter < game.getCurrentPlayer().getHand().getSize(); counter++) { // <--GET
																										// HUMAN
																										// PLAYERS
																										// HAND
			...
			int count = 0;
			for (int ind = 0; ind < game.players.length; ind++) {// need to find
																	// the
																	// indice of
																	// the
																	// current
			...
			// if there is a computer player in p1, then find = -1 and for loop
			// doesn't go
			for (int find = count; find >= 0; find--) {// find the most recent
														// human player
				// NOT WORKING WHEN P4 IS HUMAN, and P1 is computer
				if (game.players[find].isHuman()) {
					for (int counter = 0; counter < game.players[find].getHand().getSize(); counter++) {// DOES
																										// NOT
																										// GET
																										// STUCK
																										// IN
	...
	private Polygon makeScoreHex(int x, int y, int z) {
		...
		for (int a = 0; a <= 6; a++) {
			init = Math.PI / 6;
			value = Math.PI / 3.0 * a;
			hex.addPoint((int) (Math.round(x + Math.sin(value + init) * z)),
					(int) (Math.round(y + Math.cos(value + init) * z)));
		}
		return hex;
	}
		...
		for (int x = 1; x < 30; x++) {
			for (int y = 0; y < 15; y++) {
				if (!(hexagon[x][y] == null)) {
						...
						try {
							if (game.checkLegalMove(orientation, x, y)) {
								...
								score2 = game.score(game.getSecondX(orientation, x, y),
										game.getSecondY(orientation, x, y), gameBoardTempGrid);
							}
						} catch (Exception e) {
						}
					}
					...
					// if(game.getCurrentPlayer().getClass() ==
					// ComputerPlayer.class && (x ==
					// game.getCurrentPlayer().getPieceX() &&
					// y == game.getCurrentPlayer().getPieceY()) || (x ==
					// game.getSecondX(game.getCurrentPlayer().getOrientation(),
					// game.getCurrentPlayer().getPieceX(),
					// game.getCurrentPlayer().getPieceY()) &&
					// y ==
					// game.getSecondY(game.getCurrentPlayer().getOrientation(),
					// game.getCurrentPlayer().getPieceX(),
					// game.getCurrentPlayer().getPieceY()))){
	...
	public void shadeCyan(Graphics g, boolean onSpace) {
		...
		if (onSpace && game.getCurrentPlayer().getCurrentPiece() != null
				&& game.getCurrentPlayer().getClass() == HumanPlayer.class) {
			try {
				if (orientation == 0) {
	...
	private void makeBoard(int players) { // <-- Mess. Does same thing as
											// paintBoard, could be made more
											// efficient
		for (int x = 1; x < 30; x++) {
			for (int y = 0; y < 15; y++) {
				if (players == 0) {
